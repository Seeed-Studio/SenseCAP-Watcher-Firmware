...
static esp_err_t set_content_type_from_file(httpd_req_t *req, const char *filename)
{
    ...
    // {
    //     return httpd_resp_set_type(req, "audio/mp3");
    // }
...
/**
 * @brief Registers the http command.
 *
 * @return
 *          - esp_err_t
 */
esp_err_t console_cmd_http_register(void)
{
    esp_err_t ret = ESP_OK;

    esp_console_cmd_t command = { .command = "http", .help = "Command for http operations\n For more info run 'http help'", .func = &do_cmd_http };

    ret = esp_console_cmd_register(&command);
    if (ret)
    {
        ESP_LOGE(TAG, "Unable to register http");
    }
